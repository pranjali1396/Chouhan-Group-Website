      ...
      <header className={`fixed w-full z-50 transition-all duration-300 font-sans ${scrolled ? 'bg-[#002b49]/95 backdrop-blur-md shadow-md' : 'bg-[#002b49]'
        ...
        <div className="container mx-auto px-4 lg:px-6 xl:px-12">
          <div className={`flex justify-between items-center transition-all duration-300 ${scrolled ? 'h-16' : 'h-20 md:h-24'}`}>
            ...
            {/* Desktop Navigation */}
            <nav className="hidden lg:flex items-center h-full gap-3 xl:gap-6 2xl:gap-8">
              {navData.map((item) => (
                <div
                  key={item.label}
                ...
                >
                  <Link
                    to={item.path || '#'}
                    className={`flex items-center gap-1 font-heading font-bold text-[11px] xl:text-[13px] tracking-widest uppercase transition-all duration-300 py-6 border-b-2 border-transparent whitespace-nowrap ${activeMenu === item.label ? 'text-amber-400 border-amber-400' : 'text-white hover:text-amber-400'
                      }`}
                  >
                    {item.label}
                    {item.columns && (
